...
public class BookLibrary {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (menu == 1) {
				...
				String author = readStr("변경할 저자를 입력하세요[변경x > Enter]");
				String press = readStr("변경할 출판사를 입력하세요[변경x > Enter]");
				String price = readStr("변경할 가격을 입력하세요[변경x > Enter]");
	...
	public static String readStr(String msg) {
		...
		return scn.nextLine(); // enter치면 한문장을 읽어오겠다.
	}
	...
	public static int readInt(String msg) {
		...
		while (true) {
			...
			try {
				result = Integer.parseInt(val); // String타입의 val을 int로 바꿔주겠다 ("1000" > 1000)
				break;
			} catch (Exception e) {
